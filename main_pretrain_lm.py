...
from engine_pretrain_lm import train_lm_one_epoch
...
from engine_finetune import evaluate, calculate_effrank, draw_mae_predictions, calculate_cls_cls_attention


def get_args_parser():
    parser = argparse.ArgumentParser('LM pre-training', add_help=False)
    ...
    # parser.add_argument("--lpred_decoder_embed_dim", "-lded", type=int, default=None, required=False)
    # parser.add_argument("--lpred_decoder_dropout_rate", "-lddr", type=float, default=0)
    #
...
def main(args):
    ...
    if global_rank == 0 and args.log_dir is not None:
        ...
        misc.maybe_setup_wandb(args.log_dir, args=args, job_type="pretrain-lm")
    ...
    if args.input_size != 224:
        ...
        size_patch_kwargs.update(
            patch_size=args.input_size // 16
        )

    # define the model

    encoder: VisionTransformer = models_vit.__dict__[args.model](**size_patch_kwargs)
    discriminator: VisionTransformer = models_vit.VisionTransformer(
        **size_patch_kwargs,
        depth=args.discriminator_depth,
        embed_dim=512,
        num_heads=16,
    )
    ...
    model_without_ddp: LatentMixer = model
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch % args.save_interval  == 0 or epoch + 1 == args.epochs):
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp.encoder, optimizer=optimizer, loss_scaler=loss_scaler, epoch=epoch, test_stats=test_stats)
