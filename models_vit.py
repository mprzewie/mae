...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_features(
            ...
            *,
            ...
            abmilp_head: Optional[ABMILPHead] = None,
    ):
        ...
        if return_features == "raw":
            assert shuffle_subsets == 1
            ret = x_n_s_cl_d[:, 0]

        if return_features == "both":
            ret = torch.concat([x_cls, x_pos], dim=2)

        if return_features.startswith("cp"):
            # center patch represenatation
            assert shuffle_subsets==1
        ...
        if return_features.startswith("ca") or return_features == "toksec":
            # aggregate with center patch attention
            assert shuffle_subsets==1
            ca = int(return_features.split("ca")[1]) if "ca" in return_features else 1
            B, SS, T1, D = x_n_s_cl_d.shape
            x_n_cl_d = x_n_s_cl_d[:, 0]
            fm = x_n_cl_d[:, 1:]
            T = fm.shape[1]
            ...
            e = c + math.floor(ca/2)
            # fm = fm.reshape(B, hw, hw, D)
            attn_patch = attn.mean(dim=1)[:, 1:, 1:].reshape(B, hw, hw, hw**2)
            attn_ss = attn_patch[:, s:e, s:e].mean(dim=[1,2])
            attn_ss_denom = attn_ss.sum(dim=1, keepdim=True)
            attn_ss = attn_ss / (attn_ss_denom + 1e-6)

            token_selections["central_patch_attention"] = attn_ss.detach()

            ret = (fm * attn_ss.unsqueeze(2)).mean(dim=1)

        if return_features == "dino" or return_features == "toksec":
            assert shuffle_subsets == 1
            x_n_cl_d = x_n_s_cl_d[:, 0]
            fm = x_n_cl_d[:, 1:]
        ...
        if return_features.startswith("attn") or return_features == "toksec":
            assert shuffle_subsets == 1
            x_n_cl_d = x_n_s_cl_d[:, 0]
            fm = x_n_cl_d[:, 1:]
            ...
            if kind == "lcte" or return_features== "toksec": # lowest class token entropy
                ...
                token_selections["cls_attn_map_with_min_entropy"] = min_entropy_map.squeeze().detach()
                # assert False, (fm.shape, d_attn.shape)


            if kind == "mn" or return_features == "toksec": # mean class token
                mean_map = attn[:, :, 0, 1:].mean(dim=1).unsqueeze(2)
                mean_map = mean_map / mean_map.sum(dim=1, keepdim=True)
                token_selections["cls_attn_map_mean"] = mean_map.squeeze().detach()
                ret = (mean_map * fm).sum(dim=1)

        if return_features == "toksec" and abmilp_head is not None:
            _, abmilp_attn = abmilp_head.forward_with_attn_map(x_n_s_cl_d[:, 0])
            token_selections["abmilp_attn"] = abmilp_attn.squeeze().detach()


        if return_features.startswith("tcut") or return_features == "toksec":
            assert shuffle_subsets == 1
            x_n_cl_d = x_n_s_cl_d[:, 0]
            fm = x_n_cl_d[:, 1:]
            ...
            token_selections["ncut_eigen"] = (eigen / (eigen.sum(dim=1, keepdim=True) + 1e-6)).detach()
