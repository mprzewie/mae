...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        # assert False, self.pos_embed.shape
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        print("interpolate_pos_encoding", npatch, N, w, h)
    ...
    def forward(self, x, return_features: str = "cls", return_block: int = 12):
        # TODO return block ignored
        ret = self.forward_features(x)
        if return_features == "cls":
            return self.head(ret["x_norm_clstoken"])
        elif return_features == "pos":
            return  self.head(ret["x_norm_patchtokens"].mean(dim=1))
...
def vit_small(patch_size=16, num_register_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        ...
        num_heads=6,
        mlp_ratio=4,
        block_fn=Block, #partial(Block,  attn_class=Attention),
